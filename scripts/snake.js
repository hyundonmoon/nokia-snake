import { getNextDirectionAsCoordinates, getNextDirectionAsString, } from "./input.js";
...
export function updateSnake(mode) {
    SNAKE = [
        getNewSnakeHeadCoordinates(SNAKE[0], mode),
        ...SNAKE.slice(0, SNAKE.length - 1),
    ];
}
function getNewSnakeHeadCoordinates(currentHead, mode) {
    let newSnakeHeadXCoordinate = currentHead.x + getNextDirectionAsCoordinates().x;
    let newSnakeHeadYCoordinate = currentHead.y + getNextDirectionAsCoordinates().y;
    if (mode === "difficult") {
